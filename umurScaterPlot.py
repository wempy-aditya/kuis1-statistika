...
x_P = list(range(1, len(umur_P) + 1))
x_W = list(range(1, len(umur_W) + 1))

plt.figure(figsize=(8, 6))
plt.scatter(x_P, umur_P, c='blue', label='Pria')
plt.scatter(x_W, umur_W, c='red', label='Wanita')
plt.title('Scatter Plot Umur Berdasarkan Gender')
plt.xlabel('Index')
plt.ylabel('Umur')
plt.legend()
plt.grid(True)
